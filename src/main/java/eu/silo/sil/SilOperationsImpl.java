...
public class SilOperationsImpl implements SilOperations {
    ...
    /** {@inheritDoc} */
    public void createProject(String projectname, String packagename, String dbtype, String dbname,  String dbusername, String dbpassword){
        ...
        if (success) success = rooshell.executeCommand("persistence setup --provider "+jpaprovider+" --database "+dbtype+" --databaseName "+dbname+"  --userName "+dbusername+" --password "+dbpassword+" ");

        log.info("DBConfigurator created...");
        log.info("Creating Entities Layer....");
        if (success) success = rooshell.executeCommand("database reverse engineer --schema "+dbname+" --package ~.domain");
